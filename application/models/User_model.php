...
class User_model extends CI_Model {
    ...
    function validate($userArr) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $dataRes['userdata'] = $query->result();
            
            $this->db->where("user_id",$dataRes['userdata'][0]->user_id);
            $res = $this->db->get("tbl_api_key");
            if($res->num_rows()>0){
                $data = $res->result();
               
                $dataRes['userdata']['api_key'] = $data[0]->api_key;
                return $dataRes;
            }else{
                return false;
            }
...
//        echo $this->db->last_query();die;
    ...
    public function validateOldPassword($user_id, $password) {
        $count = $this->db->where(array("userid" => $user_id, "password" => $password))->from('tbl_users')->count_all_results();
        //echo $this->db->last_query();die;
        if ($count > 0) {
            return true;
        } else {
    ...
    public function updatePassword($user_id, $password) {
        ...
        $salt = salt();
        $secretIv = secretIv();
        $encPassword = encryptMyData($password, $salt, $secretIv);
        $this->db->update("users", array("password" => $encPassword, "lastupdated" => date("Y-m-d H:i:s"),
            "salt" => $salt, "secretiv" => $secretIv));
        if ($this->db->affected_rows() > 0) {
            return true;
        } else {
...
//            die;
    ...
    public function getUserList($filter = array()) {
        ...
        if (isset($filter)) {
            ...
            $query = $query . $cols . " from $this->tbl_users $cond $orderby $limit";
            $res = $this->db->query($query);
            return($res->num_rows() > 0) ? $res->result() : false;
        }
    }

    public function userListDt($filter = array()) {
        $columns = array('username', 'userid', 'nameofuser', 'usercontactno',
            'emailaddress', 'allowed_dist', 'allowed_project', 'role_id');
